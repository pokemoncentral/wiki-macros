...
(function (utils) {
  ...
  macros["learnlist template 3-4"] = function (str) {
    ...
    const num_level = (
      str.match(
        /{{Learnlist\/levelh\|([^|]+)\|(\w+)\|(\w+)\|[34]\|[1-4]}}/gi,
      ) || []
    ).length;
    if (num_level >= 2) {
      alert(
        `Found ${num_level} level learnlists, the translation for levels will be fucked up. Do it manually or ask Ff300`,
      );
    } else if (num_level === 1) {
      let gen = 0;
      ...
      const level_entries = Array.from(
        ...
        ([whole, local_gen, level, movename, rest, last_param]) => {
          ...
          if (gen === 0) {
            ...
            console.assert(
              local_gen === gen,
              `Wait: gen doesn't match: local_gen (${local_gen}) !== gen (${gen})`,
              whole,
            );
          }
          let game = "";
      ...
      for (const entry of level_entries) {
        ...
        console.assert(
          entries[movename].rest === cleaned_rest,
          entries[movename],
          cleaned_rest,
        );
      ...
      for (const [movename, entry] of Object.entries(entries)) {
        ...
        for (
          let i = 0;
          i < Math.max(...Object.values(games2levels).map((v) => v.length));
          ++i
        ) {
          const e = moduleparams[gen].map(
            (g) => games2levels[g][i] || "&mdash;",
          );
          e.push(movename);
          e.push(entry.rest);
          entries_arr.push(e);
      ...
      const sort_val = (lvl) =>
        lvl === "Inizio" ? 1 : lvl === "&mdash;" ? 101 : parseInt(lvl);
      ...
      const output =
        entries_arr.map((e) => "|" + e.join("|")).join("| //\n") + "| //";
      str = str
        .replace(
          /{{learnlist\/level([34])\|(\w+)\|([^|]+)\|([^}]*)\|([^}|]*)}}/i,
          output,
        )
        .replace(
          /{{learnlist\/level([34])\|(\w+)\|([^|]+)\|([^}]*)\|([^}|]*)}}\n?/gi,
          "",
        );
    }
