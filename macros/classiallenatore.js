...
(function (utils) {
  ...
  macros["classi allenatore VI"] = function (str) {
    return str
      .replace(/Pokémon Breeder/gi, "Allevapokémon")
  ...
  macros["trainerlist ROZA"] = function (str) {
    ...
    return (
      str
        .replace(/\{\{trainer([a-z]+)\|/gi, "{{Trainerlist/$1|")
        // sprite names
        .replace(/\|VS([a-zé\s]+)\.png/gi, "|ROZA VS$1.png")
        .replace(/ ?oras ?/gi, "")
        //rematches
        .replace(
          /After acquiring the \{\{badge\|([a-z]+)\}\}/gi,
          "Dopo aver ottenuto la {{medaglia|$1}}",
        )
        .replace(
          /After entering the \[\[Hall of Fame\]\]/g,
