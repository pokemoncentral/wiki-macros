...
(function (utils) {
  ...
  macros.langtable = function (str) {
    ...
    str = str
      ...
      .replace(
        /\|color=\{\{\#invoke\: colore \| ([\w\s\dé]*) \| (?:normale|dark|light) \}\}\n?\|bordercolor=\{\{\#invoke\: colore \| \1 \| (?:normale|dark|light) \}\}/gi,
        "|type=$1",
      )
      ...
      .replace("TCG", "GCC")
      .replace(
        /\[?\[?The Official Pok.mon Handbook\]?\]?/gi,
        "Il grande libro ufficiale dei Pok&eacute;mon",
      )
      .replace(/Games/gi, "giochi")
      .replace(
        /Pre-Gen VII Media/gi,
        "media precedenti alla settima generazione",
      )
      .replace(
        /Same as (?:its |his |hers )?English name/gi,
        "Uguale al nome inglese",
      )
      .replace(
        /Same as (?:its |his |hers )?Japanese name/gi,
        "Uguale al nome giapponese",
      )
      .replace(
        /Transliteration of (?:its |his |hers )?Japanese name/gi,
        "Traslitterazione del nome giapponese",
      )
      .replace(
        /Transcription of (?:its |his |hers )?English name/gi,
        "Trascrizione del nome inglese",
      )
      // Do those two replace really make sense?
      .replace(
        /Transliteration of (?:its |his |hers )?English name/gi,
        "Traslitterazione del nome inglese",
      )
      .replace(
        /Transcription of (?:its |his |hers )?Japanese name/gi,
    ...
    for (const lang in languages) {
      str = str
        .replace(
          new RegExp("\\|" + languages[lang][0] + " ?=([^|]*)(\\||})"),
          function (match, data, closing) {
            return (
              "|" +
              languages[lang][0] +
              "=" +
        ...
        .replace(
          new RegExp("\\|" + languages[lang][0] + "meaning ?=([^|]*)(\\||})"),
          function (match, data, closing) {
