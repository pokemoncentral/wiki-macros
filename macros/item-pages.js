...
function tabular_template(str, args) {
    ...
    const end = _end;
    const templ = args.transl_content(str.substring(begin + args.head.length, firstend));

    // Iterate if needed because of global
    const head = str.substring(0, begin) + args.head_replace + templ + args.foot_replace;
...
function sezioni(str) {
    return str
        ...
        .replace(/===Pokémon Mystery Dungeon series===/g, "===Serie Pokémon Mystery Dungeon===")
    ...
    str = tabular_template(str, {
        ...
        transl_content: function(content) {
            ...
            for (const row of content.split("|-").slice(1)) {
                ...
                cells[0] = cells[0].replace(/'''{{color2\|\s*{{[^}]*}}\s*\|\s*The Isle of Armor\s*\|\s*IoA\s*}}'''/, "IA");
